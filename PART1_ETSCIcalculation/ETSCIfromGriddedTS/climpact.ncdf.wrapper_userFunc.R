...
# This function is a conversion of the Climpact original script "climpact.ncdf.wrapper.r" (https://github.com/ARCCSS-extremes/climpact)


climpact.ncdf.wrapper_userFunc <- function (ncfile, var1, var2, var3, outpath, outFilename, 
                                            user_institution, user_contact, baseline, ncores,
                                            indList, threshFile, software_folder, nval) {
  
  #message in the original script
  # ------------------------------------------------
  ...
  vars=c(prec=var1,tmax=var2, tmin=var3)
  ...
  file.template=outFilename
  
  # author data
  author.data=list(institution=user_institution, contact = user_contact)
